...
class SubDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # d2_data = Data(x=self.d2_datas[str(id)].x, edge_index=self.d2_datas[str(id)].edge_index, edge_attr=self.d2_datas[str(id)].edge_attr) #########
...
class PubchemCapDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # atom_vec, coordinates, edge_type, dist, smiles, description, enriched_description = self.dataset[self.cid_list[index]] ############## kjh 
...
class BalanceDataset(Dataset):
    def __init__(self, root, mode, unimol_dict=None, max_atoms=256, train_datas=None):
        ...
        if mode.find('train') >= 0:
            ...
            target_path = root + '/pubchemqc/pubchemqc_database.lmdb'  ############################################ 완료
            pubchemqc_3d_conformation = D3Dataset_index(target_path, unimol_dict, max_atoms) 

            
            ### Form PubChemQC 3D Subset
            '''
            각 instruction 의 Input cid 값을 conformation 에서 불러와서 3d conformation , instruction 으로 이어붙인다
            -> conformation 을 정의할 때, 2d 를 하여 저장??? ... lmdb 파일이라 쉽지가 않다

            subdataset 할때 2d 정보 불러와서 저장?
            '''
            ...
            # target_path = '/home/kjh/kjh_dir/3d-MoLM/0415_project/data_preprocess/PubChem/3d-pubchem-all.lmdb'
